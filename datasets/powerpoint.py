...
NAVY = RGBColor(8, 18, 30)
DEEP_BLUE = RGBColor(13, 29, 43)
...
PALE = RGBColor(25, 52, 70)
BACKGROUND = RGBColor(8, 18, 30)
PANEL = RGBColor(14, 32, 47)
PANEL_LIGHT = RGBColor(18, 41, 58)
PANEL_ALT = RGBColor(16, 37, 53)
PANEL_BORDER = RGBColor(35, 70, 91)
FONT_NAME = 'IBM Plex Sans Arabic'
PPT_CHART_PALETTE = ('#2563EB', '#38BDF8', '#2DD4BF', '#22C55E', '#60A5FA')
...
def add_title(slide, title):
    box = _textbox(slide, title, Inches(.55), Inches(.28), Inches(10.1), Inches(.62), TITLE_SIZE, True, WHITE)
    box.name = 'SLIDE_TITLE'
    rule = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(9.55), Inches(.94), Inches(1.1), Inches(.035))
    rule.fill.solid(); rule.fill.fore_color.rgb = SKY; rule.line.fill.background()
    return box


def add_subtitle(slide, text, top=1.05):
    box = _textbox(slide, text, Inches(.58), Inches(top), Inches(10.05), Inches(.42), SUBTITLE_SIZE, False, SLATE)
...
def add_modern_footer(slide, number):
    line = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(.55), Inches(6.96), Inches(10.1), Inches(.015))
    line.fill.solid(); line.fill.fore_color.rgb = PANEL_BORDER; line.line.fill.background()
    footer = _textbox(slide, 'Analytix • Data Intelligence', Inches(.55), Inches(7.05), Inches(2.6), Inches(.22), 10, True, SLATE, PP_ALIGN.LEFT)
...
def add_insight_panel(slide, text):
    panel = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(.55), Inches(6.02), Inches(10.1), Inches(.76))
    ...
    line = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(10.52), Inches(6.14), Inches(.035), Inches(.5))
    line.fill.solid(); line.fill.fore_color.rgb = SKY; line.line.fill.background()
    _textbox(slide, 'القراءة الرئيسية', Inches(8.45), Inches(6.17), Inches(1.82), Inches(.3), 13, True, SKY)
    _textbox(slide, text[:250], Inches(.82), Inches(6.15), Inches(7.35), Inches(.4), 12, False, WHITE)
    return panel
...
def add_chart_slide_modern(slide, chart, insight='', metrics=None):
    ...
    for index, (label, value, color) in enumerate(metrics[:4]):
        left = Inches(.55 + index * 2.55)
        card = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, left, Inches(1.12), Inches(2.38), Inches(.76))
        card.name = 'CHART_KPI'; card.fill.solid(); card.fill.fore_color.rgb = PANEL
        card.line.color.rgb = PANEL_BORDER; card.line.width = Pt(.5)
        _textbox(slide, str(value), left + Inches(.12), Inches(1.2), Inches(.9), Inches(.4), 19, True, color, PP_ALIGN.LEFT)
        _textbox(slide, label, left + Inches(.94), Inches(1.24), Inches(1.26), Inches(.3), 10, False, SLATE)
    chart_width = Inches(10.1)
    chart_left = Inches(.55)
...
def add_clean_table(slide, rows, left, top, width, row_height=Inches(.72)):
    ...
    container = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, left, top, width, row_height * (min(5, len(rows)) + 1))
    container.name = 'MODERN_TABLE'; container.fill.solid(); container.fill.fore_color.rgb = PANEL
    container.line.color.rgb = PANEL_BORDER; container.line.width = Pt(.5)
    header = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, left + Inches(.04), top + Inches(.04), width - Inches(.08), row_height - Inches(.08))
    header.fill.solid(); header.fill.fore_color.rgb = BLUE; header.line.fill.background()
    _textbox(slide, 'المشكلة', left + int(width * .38), top + Inches(.14), int(width * .57), Inches(.34), 13, True, WHITE)
    _textbox(slide, 'القيمة', left + int(width * .18), top + Inches(.14), int(width * .18), Inches(.34), 13, True, WHITE, PP_ALIGN.CENTER)
    _textbox(slide, 'الأولوية', left + Inches(.08), top + Inches(.14), int(width * .14), Inches(.34), 13, True, WHITE, PP_ALIGN.CENTER)
    for index, row in enumerate(rows[:5]):
        y = top + (index + 1) * row_height
        if index % 2 == 0:
            stripe = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, left + Inches(.04), y, width - Inches(.08), row_height)
            stripe.fill.solid(); stripe.fill.fore_color.rgb = PANEL_ALT; stripe.line.fill.background()
        _textbox(slide, row[0], left + int(width * .38), y + Inches(.14), int(width * .57), Inches(.38), 14, False, WHITE)
        _textbox(slide, row[1], left + int(width * .18), y + Inches(.14), int(width * .18), Inches(.38), 14, True, SKY, PP_ALIGN.CENTER)
        _textbox(slide, row[2], left + Inches(.08), y + Inches(.14), int(width * .14), Inches(.38), 12, True, severity_colors.get(row[2], SLATE), PP_ALIGN.CENTER)
        dot = slide.shapes.add_shape(
            MSO_SHAPE.OVAL, left + Inches(.12), y + Inches(.27), Inches(.09), Inches(.09)
        )
...
def add_priority_columns(slide, columns):
    ...
    for index, (title, items) in enumerate(columns):
        left = Inches(.55 + index * 3.39)
        add_section_card(slide, left, Inches(1.72), Inches(3.16), Inches(4.72), title, colors[index])
        add_badge(slide, f'{len(items)} توصيات', left + Inches(.18), Inches(2.22), colors[index])
        for item_index, item in enumerate(items[:3]):
            dot = slide.shapes.add_shape(MSO_SHAPE.OVAL, left + Inches(2.83), Inches(2.98 + item_index * 1.02), Inches(.1), Inches(.1))
            dot.fill.solid(); dot.fill.fore_color.rgb = colors[index]; dot.line.fill.background()
            _textbox(slide, str(item)[:130], left + Inches(.18), Inches(2.84 + item_index * 1.02), Inches(2.5), Inches(.72), 13, False, WHITE)
...
def _base_slide(prs, title, number):
    ...
    glow = slide.shapes.add_shape(MSO_SHAPE.OVAL, Inches(1.7), Inches(.35), Inches(7.4), Inches(6.5))
    glow.name = 'SOFT_GLOW'; glow.fill.solid(); glow.fill.fore_color.rgb = RGBColor(18, 64, 83)
    glow.fill.transparency = 72; glow.line.fill.background()
    add_modern_header(slide, prs)
    add_dashboard_sidebar(slide, 'تقرير تحليل البيانات', 'الرسوم')
    add_title(slide, title)
    add_modern_footer(slide, number)
    return slide
...
def _dashboard_chart(labels, values, kind='bar', colors_list=None):
    ...
    try:
        ...
        figure, axis = plt.subplots(figsize=(12, 4.4), facecolor='#0E202F')
        axis.set_facecolor('#0E202F')
        ...
        if kind in ('pie', 'donut'):
            wedges, _, autotexts = axis.pie(
                ...
                wedgeprops={'width': .38, 'edgecolor': '#0E202F', 'linewidth': 3},
            )
            ...
            axis.legend(wedges, display_labels, frameon=False, labelcolor='#CBD5E1',
                        loc='lower center', bbox_to_anchor=(.5, -.08), ncol=min(4, len(display_labels)),
                        fontsize=9, prop=chart_font)
            axis.axis('equal')
        elif kind == 'horizontal':
            positions = range(len(values))
            bars = axis.barh(positions, values, color=[palette[i % len(palette)] for i in positions], height=.55)
            axis.set_yticks(list(positions)); axis.set_yticklabels(display_labels)
            axis.invert_yaxis(); axis.grid(axis='x', color='#23465B', linewidth=.6, alpha=.55)
            ...
            axis.plot(positions, values, color=palette[1 % len(palette)], linewidth=3,
                      marker='o', markersize=7, markerfacecolor=palette[3 % len(palette)])
            axis.fill_between(positions, values, color=palette[1 % len(palette)], alpha=.10)
            axis.set_xticks(positions); axis.set_xticklabels(display_labels)
            axis.grid(axis='y', color='#23465B', linewidth=.6, alpha=.55)
            for index, value in enumerate(values):
                axis.annotate(f'{value:,.1f}', (index, value), xytext=(0, 9), textcoords='offset points',
                              ha='center', color='#E2E8F0', fontsize=9, fontproperties=chart_font)
        elif kind == 'scatter':
            positions = list(range(len(values)))
            sizes = [70 + (abs(float(value)) % 5) * 18 for value in values]
            axis.scatter(positions, values, s=sizes, color=palette[1 % len(palette)], alpha=.85,
                         edgecolor='#2DD4BF', linewidth=.8)
            axis.set_xticks(positions); axis.set_xticklabels(display_labels)
            axis.grid(color='#23465B', linewidth=.6, alpha=.55)
        else:
            positions = range(len(values))
            bars = axis.bar(positions, values, color=[palette[i % len(palette)] for i in positions], width=.55)
        ...
        figure.subplots_adjust(
            left=.07, right=.97,
            bottom=.27 if len(values) > 5 and kind not in ('pie', 'donut', 'horizontal') else .16,
            top=.95,
        )
        figure.savefig(buffer, format='png', dpi=300, facecolor='#0E202F')
        buffer.seek(0)
        return buffer
    except Exception:
        buffer.close()
...
def _dashboard_widget_chart(widget, report):
    try:
        ...
        return _dashboard_chart(
            ...
            {'pie': 'donut', 'line': 'line'}.get(widget['type'], 'bar'),
        )
    except Exception:
        return None
...
def _chart_slide(prs, title, labels, values, number, kind='bar', palette=None):
    ...
    chart_kind = {'pie': 'donut', 'barh': 'horizontal'}.get(kind, kind)
    chart = _dashboard_chart(short_labels, values[:8], chart_kind, chart_colors)
    if not chart:
        return False
    try:
...
def _dashboard_gauge(value):
    ...
    try:
        ...
        chart_font = _chart_font()
        figure, axis = plt.subplots(figsize=(3.2, 2.5), facecolor='#0E202F')
        axis.set_facecolor('#0E202F')
        axis.pie([value, 100 - value], startangle=90, counterclock=False,
                 colors=['#41D39A', '#3B6A82'],
                 wedgeprops={'width': .24, 'edgecolor': '#0E202F', 'linewidth': 2})
        ...
        figure.savefig(buffer, format='png', dpi=300, facecolor='#0E202F', bbox_inches='tight')
...
def build_powerpoint_report(dataset, report):
    ...
    quality_cards = [
        ...
        ('الجودة العامة', f'{summary["quality_score"]}/100'),
    ]
    for index, (label, value) in enumerate(quality_cards):
        _kpi_card(slide, label, value, Inches(.55 + index * 2.55), Inches(1.12), [TEAL, RED, AMBER, GREEN][index], width=Inches(2.38))
    quality_chart = _dashboard_chart(
        [item['name'] for item in report['sheets'][:8]],
        [item['quality_score'] for item in report['sheets'][:8]], 'bar', list(PPT_CHART_PALETTE),
    )
    completion_chart = _dashboard_chart(
        ['مكتملة', 'فارغة'], [summary['completion_rate'], max(0, 100 - summary['completion_rate'])],
        'donut', ['#2DD4BF', '#23465B'],
    )
    quality_gauge = _dashboard_gauge(summary['quality_score'])
    try:
        if quality_chart: add_image_panel(slide, quality_chart, Inches(.55), Inches(2.12), Inches(4.65), Inches(3.7), 'QUALITY_BAR')
        if completion_chart: add_image_panel(slide, completion_chart, Inches(5.38), Inches(2.12), Inches(2.52), Inches(3.7), 'COMPLETION_DONUT')
        if quality_gauge: add_image_panel(slide, quality_gauge, Inches(8.07), Inches(2.12), Inches(2.58), Inches(3.7), 'QUALITY_GAUGE')
    finally:
        if quality_chart: quality_chart.close()
        if completion_chart: completion_chart.close()
        if quality_gauge: quality_gauge.close()
    add_insight_panel(slide, f'بلغت الجودة العامة {summary["quality_score"]}% مع اكتمال قدره {summary["completion_rate"]}%؛ وتستحق الأوراق الأقل جودة أولوية المراجعة.')

    issues = report['executive_summary']['issues']
    slide = _base_slide(prs, 'أهم المشكلات', len(prs.slides) + 1)
    issue_metrics = [
        ('إجمالي المشكلات', len(issues), RED),
        ('مرتفعة', sum(1 for item in issues if item['severity'] == 'مرتفع'), RED),
        ('متوسطة', sum(1 for item in issues if item['severity'] == 'متوسط'), AMBER),
        ('منخفضة', sum(1 for item in issues if item['severity'] == 'منخفض'), TEAL),
    ]
    for index, (label, value, color) in enumerate(issue_metrics):
        _kpi_card(slide, label, value, Inches(.55 + index * 2.55), Inches(1.12), color, width=Inches(2.38))
    if issues:
        rows = []
        for issue in issues[:5]:
            number = issue['issue'].split(':', 1)[-1].strip()[:30]
            rows.append((f'{issue["sheet"]}: {issue["issue"].split(":", 1)[0]}', number, issue['severity']))
        add_clean_table(slide, rows, Inches(.55), Inches(2.16), Inches(10.1), row_height=Inches(.62))
    else:
        add_bullet_panel(slide, 'حالة الجودة', ['لم تُرصد مشكلات بارزة وفق قواعد التقرير.'], Inches(.55), Inches(2.16), Inches(10.1), Inches(3.35), color=TEAL)
    add_insight_panel(slide, 'ابدأ بمعالجة المشكلات مرتفعة الأولوية، ثم راجع القيم الفارغة والتكرارات قبل مشاركة النتائج النهائية.')
    ...
    for widget in report['widgets']:
        ...
        if widget['type'] in ('bar', 'line', 'pie') and chart_count < MAX_POWERPOINT_CHARTS:
            ...
            slide = _base_slide(prs, widget['title'], len(prs.slides) + 1)
            details = [('نوع العنصر', widget['type'], BLUE), ('الورقة', widget['sheet'] or 'غير محددة', SKY), ('التجميع', widget['aggregation'] or 'بدون', TEAL)]
            for index, (label, value, color) in enumerate(details):
                _kpi_card(slide, label, value, Inches(.55 + index * 3.39), Inches(1.2), color, width=Inches(3.16))
            add_bullet_panel(slide, 'قراءة العنصر', ['هذا العنصر محفوظ في لوحة المعلومات الحالية ويعتمد على البيانات الفعلية للورقة المحددة.', 'يمكن استخدامه كنقطة بداية للمقارنة والمتابعة الدورية.'], Inches(.55), Inches(2.75), Inches(10.1), Inches(3.05), max_items=2, color=TEAL)
            add_insight_panel(slide, 'يُلخّص العنصر إعدادات المؤشر المحفوظ ويُبقي سياقه مرتبطًا بالورقة ومصدر البيانات الأصليين.')
    ...
    for sheet in report['sheets']:
        ...
        for index, (label, value, color) in enumerate(sheet_cards):
            _kpi_card(slide, label, value, Inches(.55 + index * 2.55), Inches(1.12), color, width=Inches(2.38))
        missing_items = [item for item in sheet['missing'] if item['count']][:8]
        if missing_items:
            sheet_chart = _dashboard_chart([item['column'] for item in missing_items], [item['count'] for item in missing_items], 'horizontal')
            insight = f'تتركز القيم الفارغة في {missing_items[0]["column"]} بعدد {missing_items[0]["count"]:,}؛ راجع هذا العمود أولًا. '
        else:
            sheet_chart = _dashboard_chart(
                ['الصفوف', 'الأعمدة', 'القيم الفارغة', 'المكررة'],
                [sheet['rows'], sheet['columns'], sheet['missing_cells'], sheet['duplicates']], 'bar', list(PPT_CHART_PALETTE),
            )
            insight = 'لا توجد قيم فارغة بارزة؛ استمر في المراجعة الدورية للتكرارات وصحة أنواع الأعمدة. '
        try:
            if sheet_chart:
                add_image_panel(slide, sheet_chart, Inches(.55), Inches(2.08), Inches(10.1), Inches(3.72), 'SHEET_DASHBOARD_CHART')
        finally:
            if sheet_chart: sheet_chart.close()
        recommendation = sheet['recommendations'][0] if sheet.get('recommendations') else 'حافظ على ضوابط الجودة الحالية وراجع النتائج دوريًا.'
        add_insight_panel(slide, f'{insight} التوصية: {recommendation}')
